...
3. For each point: check Street View **metadata** first (free) to skip points with no panorama coverage, then fetch 9 Street View images — 4 wide (90° FOV) establishing shots N/E/S/W for orientation, 4 closer/steeper (50° FOV, -45° pitch) shots at the same headings to actually resolve pavement-scale detail (a wide shallow-pitch shot alone makes a small pothole nearly invisible), and one steep (-80° pitch) straight-down shot.
...
If you have confirmed real-world examples (or clear negatives) from manually browsing Street View, sharing the exact images/descriptions is far more useful for calibrating this than further guessing at prompt wording.
