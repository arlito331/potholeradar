...
3. For each point: check Street View **metadata** first (free) to skip points with no panorama coverage, then fetch 9 Street View images (8 compass headings + downward tilt — 8 rather than 4 so the road's actual direction, whatever it is, is very unlikely to be missed entirely).
